...
/* The chooser's tree.  NOT in G: the gates compare G byte for byte, and a
 * pointer the desktop looks up once at start-up has no business widening
 * a structure the model has to mirror. */
static OBJECT *a_pref;
...
int main(void)
{
    ...
    rsrc_gaddr(R_TREE, ADPREF, (void **)&a_pref);
