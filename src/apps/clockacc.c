 ...
 * this accessory took nothing from that program -- its resource is its
 * own, from before the program existed, and its workstation is opened and
 * closed around each panel rather than held.  It must not close its own
 * window either: the AES does that, and a handle kept across an AC_CLOSE
 * is a handle to a window that no longer exists.
 */
#include "gem.h"
