...
| `make test-m4` | 14/14 | AES object library: draw, find, change, edit, centre, icons, and the tree surgery an application does at run time — `objc_add` and `objc_delete` through every defined branch of the child chain (a middle child, the head, the tail, the last one left, a first child again, and the root refusing to go), checked in the tree's own memory as well as on the screen |
